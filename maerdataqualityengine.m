function [Structed_DQ_Set] = maerdataqualityengine(varargin)
...
%	
% Made by Amang Kim [v0.4b || 3/3/2019]
...
for i1=1:NumofId %-----------------------------------------------
    ...
    [maer0, AER00] = maer(dm, TestingECG);
    
    

%sC3 = rangecontrolmulti(mse0);
sC3 = rangecontrol(maer0);
%-----------------------------
MAER0 = mean (maer0);
APR0 = sC3.APR;
UCL0 = mean(sC3.UCL);
...
SDQS.MAER = MAERBar;
SDQS.min_mean_Max_MAER_APR = [min(APR) mean(APR) max(APR)];
SDQS.min_mean_Max_MAER_APU = [min(APU) mean(APU) max(APU)];
SDQS.min_mean_Max_MAER_UCL = [min(UCL) mean(UCL) max(UCL)];
